...
from evaluator.allocation import Allocation
...
def evaluate(unicast_list, graph):
    ...
    for unicast in unicast_list:
        if isinstance(unicast.get_app(), TTApplication):
            for edge in unicast.get_path():
                if edge in tt_allocation_dict:
                    total_message_size_mbps = unicast.get_app().get_message_size_mbps() + tt_allocation_dict[edge][0].get_message_size_mbps()
                    tt_allocation_dict[edge][0].set_message_size_mbps(total_message_size_mbps)
                else:
                    ...
                    tt_allocation_dict[edge] = allocation_list
