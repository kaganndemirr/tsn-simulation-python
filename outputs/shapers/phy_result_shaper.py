...
class PHYResultShaper:
    def __init__(self, phy_holder):
        self.topology_output_location = os.path.join("results", phy_holder.get_routing(), phy_holder.get_path_method(), phy_holder.get_algorithm(), str(phy_holder.get_k()), phy_holder.get_topology_name() + "_" +  phy_holder.get_application_name())
        self.result_output_location = os.path.join("results", phy_holder.get_routing(), phy_holder.get_path_method(), phy_holder.get_algorithm(), str(phy_holder.get_k()))
    ...
    def write_srt_unicast_candidate_list(self, srt_unicat_candidate_list):
        srt_unicast_candidate_list_path = os.path.join(self.topology_output_location, "AVBCandidateRoutes.txt")
        with open(srt_unicast_candidate_list_path, 'w') as srt_unicast_candidate_list_writer:
            for unicast_candidate in srt_unicat_candidate_list:
                for candidate_path in unicast_candidate.get_candidate_path_list():
                    srt_unicast_candidate_list_writer.write(f"{unicast_candidate.get_application().get_name()}\t{candidate_path}\n")
