...
class GRASP:
    def __init__(self, tt_unicast_list, k):
        self.k = k
        ...
        self.non_tt_unicast_candidate_list = None
        self.tt_unicast_list = tt_unicast_list
    ...
    def run_grasp(self, thread_name):
        ...
        while self.running:
            initial_solution = construct_initial_solution(self.non_tt_unicast_candidate_list, self.tt_unicast_list, self.k, self.evaluator)
            solution = grasp(initial_solution, self.evaluator, self.non_tt_unicast_candidate_list, self.global_best_cost)
    ...
    def solve(self, graph, application_list, path_finding_method, evaluator, thread_number, timeout):
        yen_k_shortest_paths_start = time.time()
        yen_k_shortest_paths = YenKShortestPath(graph, application_list, path_finding_method, self.k)
        yen_k_shortest_paths_end = time.time()

        self.yen_k_shortest_path_time = yen_k_shortest_paths_end - yen_k_shortest_paths_start

        self.non_tt_unicast_candidate_list = yen_k_shortest_paths.get_srt_unicast_candidate_list()
    ...
    def get_non_tt_unicast_candidate_list(self):
